...
- Permission justification for `activeTab`: read the current tab selection only after the user chooses the context menu item or keyboard shortcut.
...
- Host permission justification: communicate with `https://app.scholarmark.ai` for authentication, project listing, API-key revocation, and clip creation.
