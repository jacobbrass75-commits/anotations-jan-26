          ...
          <div className="space-y-4">
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <Label>
                  Select Documents ({selectedIds.size}/{documents.length})
                </Label>
                <div className="flex gap-2">
                  <Button
                    variant={selectedIds.size === documents.length ? "secondary" : "outline"}
                    size="sm"
            ...
            <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
              <CollapsibleTrigger asChild>
                <Button
                  variant="ghost"
                  className="w-full justify-between uppercase tracking-wider text-xs"
            ...
            {(isAnalyzing || selectedIds.size > 0) && (
              <div className="rounded-md border bg-muted/30 p-3 space-y-2">
                <div className="flex items-center justify-between gap-3 text-xs text-muted-foreground">
                  ...
                  <span>
                    {isAnalyzing ? `${elapsedSeconds}s elapsed` : `${progressValue}% ready`}
                  </span>
                </div>
          ...
          <div className="space-y-4">
            <Alert variant={response.status === "failed" ? "destructive" : "default"}>
              ...
              <AlertDescription>
                ...
                created {response.totalAnnotationsCreated} annotations in{" "}
                {(response.totalTimeMs / 1000).toFixed(1)}s
              </AlertDescription>
            </Alert>
