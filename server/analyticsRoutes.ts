...
export function registerAnalyticsRoutes(app: Express): void {
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (format === "csv") {
          return res
            .status(400)
            .json({ message: "CSV export is not implemented. Use format=json." });
        }
        ...
        const toolCallsTotal =
          (selectToolCallTotal.get(from, to) as CountRow | undefined)?.total ?? 0;
        const contextSnapshotsTotal =
          (selectContextSnapshotTotal.get(from, to) as CountRow | undefined)?.total ?? 0;
  ...
  app.get(
    "/api/admin/analytics/conversation/:id",
    requireAuth,
    requireAdmin,
    async (req: Request, res: Response) => {
      try {
        ...
        const contextSnapshots = selectConversationContextSnapshots.all(
          conversationId,
