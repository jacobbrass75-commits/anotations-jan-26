...
import { processChunksWithPipelineV2, clearDocumentContextCacheV2 } from "./pipelineV2";
...
async function getOwnedDocumentOr404(req: Request, res: Response): Promise<Document | null> {
...
export async function registerRoutes(httpServer: Server, app: ExpressApp): Promise<Server> {
  ...
  app.get("/api/documents/:id/source", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      if (!available) {
        return res
          .status(404)
          .json({ message: "Original source file is not available for this document" });
      }
    ...
    async (req: Request, res: Response) => {
      ...
      try {
        ...
        if (doc.status === "error") {
          return res
            .status(409)
            .json({ message: "Document processing failed. Please re-upload the document." });
        }
  ...
  app.post("/api/documents/:id/annotate", requireAuth, async (req: Request, res: Response) => {
    try {
      const { startPosition, endPosition, highlightedText, category, note, isAiGenerated } =
        req.body;
